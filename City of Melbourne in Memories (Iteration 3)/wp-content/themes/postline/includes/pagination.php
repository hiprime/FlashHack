...
if ( 'infinite' == themify_get( 'setting-more_posts' ) || '' == themify_get( 'setting-more_posts' ) ) {
	...
	if ( $total_pages > $current_page ) {
		...
		if ( $themify->post_layout == 'timeline' || ( 'timeline' == $themify->post_layout && is_page() ) ) {
			// do nothing
			// timeline pagination is different than regular so it's need to always show outside total pages
		} else {
			echo '<div id="general-load-more"><p id="load-more"><a href="' . next_posts( $total_pages, false ) . '">' . __( 'Load More', 'themify' ) . '</a></p></div>';
		}
	}
	if ( $themify->post_layout == 'timeline' || ( 'timeline' == $themify->post_layout && is_page() ) ): ?>
